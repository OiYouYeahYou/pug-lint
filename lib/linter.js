...
class Linter {
	...
	configure(options) {
		...
		if (options) {
			...
			if (
				Object.prototype.hasOwnProperty.call(options, 'additionalRules')
			) {
				assert(
	...
	_isFileExcluded(filePath) {
		...
		return this._excludedFileMatchers.some((matcher) =>
			matcher.match(filePath)
		);
	}

	_loadExcludedFiles() {
		this._excludedFileMatchers = this._excludedFileMasks.map(
			(fileMask) =>
				new minimatch.Minimatch(
					path.resolve(this._basePath, fileMask),
					{
						dot: true
					}
				)
	...
	_resolveExtendsFile(filePath) {
		if (isAbsolutePath(filePath) || !/\w|@/.test(filePath.charAt(0))) {
			...
			try {
				...
				throw new Error(
					'Cannot find module "' + packageName + '" to extend'
				);
			}
		}
